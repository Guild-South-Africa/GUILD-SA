...
// Wave background inspired by the original CanvasRenderer particle grid.
const waveSettings = {
  separation: 10,
  amountX: 104,
  amountY: 72,
...
const waveParticleCount = waveSettings.amountX * waveSettings.amountY
const wavePositions = new Float32Array(waveParticleCount * 3)
const waveOrigin = new Float32Array(waveParticleCount * 3)

let waveIndex = 0
for (let ix = 0; ix < waveSettings.amountX; ix++) {
  for (let iy = 0; iy < waveSettings.amountY; iy++) {
    const x = ix * waveSettings.separation - (waveSettings.amountX * waveSettings.separation) / 2
    const z = iy * waveSettings.separation - (waveSettings.amountY * waveSettings.separation) / 2 - 180
    const y = -130
    const offset = waveIndex * 3
    wavePositions[offset] = x
    wavePositions[offset + 1] = y
    wavePositions[offset + 2] = z
    waveOrigin[offset] = x
    waveOrigin[offset + 1] = y
    waveOrigin[offset + 2] = z
    waveIndex++
  }
}
...
function buildGUI() {
  ...
  addFolder('Books', [
    ...
    { name: 'Flap Speed', key: 'tailSpeed', type: 'range', min: 0, max: 30, step: 0.5, onChange: (v) => { boids.tailSpeed = v } },
    { name: 'Book Color', key: 'fishColor', type: 'color', onChange: (v) => { boids.fishColor = v } },
  ])
...
const wavePointer = new THREE.Vector2(85, -342)
...
addEventListener('pointermove', (e) => {
  ...
  wavePointer.x = e.clientX - windowHalfX
  wavePointer.y = e.clientY - windowHalfY * 2
})

addEventListener('touchmove', (e) => {
  if (e.touches.length !== 1) return
  wavePointer.x = e.touches[0].pageX - windowHalfX
  wavePointer.y = e.touches[0].pageY - windowHalfY * 2
}, { passive: true })

function updateWaveBackground(time) {
  ...
  for (let ix = 0; ix < waveSettings.amountX; ix++) {
    for (let iy = 0; iy < waveSettings.amountY; iy++) {
      ...
      positions[offset + 1] = waveOrigin[offset + 1]
        + Math.sin((ix + time) * 0.3) * waveSettings.amplitude
        + Math.sin((iy + time) * 0.5) * waveSettings.amplitude
      index++
    }
  }

  wave.rotation.x += (((-wavePointer.y / Math.max(windowHalfY, 1)) * 0.08) - wave.rotation.x) * 0.045
  wave.rotation.y += (((wavePointer.x / Math.max(windowHalfX, 1)) * 0.1) - wave.rotation.y) * 0.045
...
renderer.setAnimationLoop(async () => {
  ...
  updateWaveBackground(now * 0.001 * waveSettings.speed)
  boids.update(delta)
